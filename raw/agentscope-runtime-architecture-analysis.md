...
> 仓库：https://github.com/agentscope-ai/agentscope-runtime · 分析日期：2026-06-12 · 版本：HEAD `22072fd`（2026-06-04，fix(engine): use default_factory for BaseResponse.created_at (#512)）· 获取方式：GitHub API 复核 HEAD + codeload tarball 源码扫描。
...
| `adapters/**` | AgentScope、LangGraph、MS Agent Framework、Agno、AutoGen 等消息/工具适配层。 |
...
4. 部署路径由 `DeployManager` 抽象，开发态可以 local daemon，生产态可转成 K8s/Knative/FC。
...
- 仓库生命周期风险明显：README archive notice 表示新项目应优先看 AgentScope 2.0，本页价值在理解 runtime 抽象如何被并入 framework。
