# GitHub Stars P2 snapshot: NVIDIA/k8s-device-plugin

> Repository: https://github.com/NVIDIA/k8s-device-plugin
> Snapshot date: 2026-06-12
> Source: GitHub REST API `repos/NVIDIA/k8s-device-plugin`

- Full name: `NVIDIA/k8s-device-plugin`
- Description: NVIDIA device plugin for Kubernetes
- Default branch: `main`
- Language: Go
- License: Apache-2.0
- Stars: 3786
- Forks: 830
- Open issues: 76
- Last push: 2026-06-11T23:13:06Z
- Archived: false
- Fork: false
- Topics: kubernetes

## Wiki use

Use this as the raw current-state snapshot for NVIDIA Kubernetes device plugin. Future ingest should inspect device discovery, allocation, MIG/sharing configuration, CDI strategy, and relation to GPU Operator.
