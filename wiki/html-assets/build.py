...
def build_site_index(resolver: Resolver) -> str:
    ...
    entries = []
    for cat in CATEGORIES:
        for f in sorted((WIKI / cat).glob("*.md") if (WIKI / cat).is_dir() else []):
            p = load_page(f, cat)
            tagstr = ",".join(p.fm.tags)
            entries.append(f'{{"t":"{htmllib.escape(p.title)}","h":"{cat}/{f.stem}.html","g":"{htmllib.escape(tagstr)}","c":"{cat}"}}')
    search_json = "[" + ",".join(entries) + "]"
...
<script>
  ...
  function search(q) {{
    ...
    const hits = SEARCH_INDEX.filter(e =>
      e.t.toLowerCase().includes(q) || e.g.toLowerCase().includes(q)
    ).slice(0, 30);
    out.innerHTML = hits.map(e =>
      `<li><a class="wikilink" href="${{e.h}}">${{e.t}}</a><small>${{e.c}} · ${{e.g}}</small></li>`
    ).join("");
  }}
