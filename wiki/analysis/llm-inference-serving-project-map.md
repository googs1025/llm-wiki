...
- benchmark / simulator：[[llm-d-benchmark]]、[[llm-d-inference-sim]] 已有，后续补 `inference-perf` 和 [[llm-d]] Prism。
- autoscaling / queueing：[[llm-d-workload-variant-autoscaler]] 已有，后续补 `Kueue`、`Karpenter`、`metrics-server`、`prometheus-adapter`。
