...
date: 2026-06-09
sources: [src-dynamo-architecture, src-sglang-architecture, src-skypilot-architecture, src-k8s-gpu-device-plugins-stars, src-llm-d-architecture]
related: [[dynamo]], [[vllm]], [[sglang]], [[llm-d]], [[llm-inference]], [[paged-attention]], [[radix-attention]], [[disaggregated-serving]], [[kv-cache-offload]], [[kubernetes]], [[llm-d-kubernetes-sigs-candidate-map]]
---
...
详见 [[llm-d-kubernetes-sigs-candidate-map]]。当前最值得补的方向不是再加一个推理 engine，而是补全 serving control plane 的外围能力：

- batch inference：`llm-d-batch-gateway`。
- benchmark / simulator：`llm-d-benchmark`、`llm-d-inference-sim`、`inference-perf`。
- autoscaling / queueing：`llm-d-workload-variant-autoscaler`、`Kueue`、`Karpenter`。
- distributed workload API：`LeaderWorkerSet`、`JobSet`。
...
- 还缺少 `llm-d-batch-gateway`、`llm-d-benchmark`、`llm-d-inference-sim`，无法完整解释 [[llm-d]] 的 batch、评测和仿真外围。
