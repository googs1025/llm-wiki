...
| P0 | `kubebuilder` | CRD / controller 脚手架 | 和 controller-runtime 一起构成 Operator 工程主线。 |
| P0 | `metrics-server` | 可观测 / autoscaling | HPA/VPA 基础指标源，解释 autoscaling 体系必需。 |
| P0 | `external-dns` | 网络 / DNS | K8s service/ingress/gateway 到 DNS record 的控制器代表。 |
| P0 | `secrets-store-csi-driver` | 存储 / 凭据 | CSI 方式注入外部 secret，连接凭据治理和 runtime security。 |
| P0 | `kind` | 计算 / 测试集群 | 本地 K8s 测试底座，很多 controller/operator 项目的开发环境基线。 |
...
| 项目 | 优先级 | 为什么值得加入 |
|---|---|---|
| `external-dns` | P0 | 从 Service/Ingress/Gateway 等 K8s 对象动态维护外部 DNS，是典型“声明式网络控制器”。 |
| `aws-load-balancer-controller` | P0 | AWS LB / Ingress / Gateway API 的生产代表，适合和 [[gateway-api]] 放在同一条网络入口路线。 |
| [[gateway-api]] | 已有 | 已收录，作为新一代流量入口 API 基础。 |
| [[gateway-api-inference-extension]] | 已有 | 已收录，作为 LLM inference endpoint picking 标准层。 |
| `ingress2gateway` | P1 | 从 Ingress 迁移到 Gateway API 的工具，适合补迁移路线。 |
| `apiserver-network-proxy` | P1 | control plane 到节点网络代理，适合理解托管 K8s / 私有网络控制面连接。 |
| `kube-agentic-networking` | P1 | Agent/tool networking policy governance，和 [[agent-sandbox]]、[[agentgateway]] 方向高度相关。 |
...
| 项目 | 优先级 | 为什么值得加入 |
|---|---|---|
| `secrets-store-csi-driver` | P0 | 把外部 secret store 通过 CSI volume 注入 Pod，连接 [[agent-credential-isolation]] 和 [[cloud-native-security]]。 |
| `aws-ebs-csi-driver` | P0 | 块存储 CSI driver 的云厂商代表。 |
| `aws-efs-csi-driver` | P0 | 文件存储 CSI driver 的云厂商代表，适合与 EBS 对比。 |
| `nfs-subdir-external-provisioner` | P1 | 轻量 NFS 动态 PV，适合本地/实验集群。 |
| `sig-storage-local-static-provisioner` | P1 | local PV 静态 provisioner，适合理解本地盘和调度绑定。 |
| `sig-storage-lib-external-provisioner` | P1 | CSI/external provisioner 的库和控制器抽象。 |
...
| 项目 | 优先级 | 为什么值得加入 |
|---|---|---|
| `kueue` | P0 | K8s-native Job queueing，是 AI/HPC/batch workload 多租户队列核心。 |
| `karpenter` | P0 | 节点级弹性和容量经济，适合接 LLM serving 的成本与 SLO。 |
| `scheduler-plugins` | P0 | kube-scheduler framework 的 out-of-tree 插件集合，理解调度扩展必读。 |
| `descheduler` | P1 | 通过策略触发重调度，补“调度后优化”维度。 |
| `kwok` | P1 | 无 kubelet 模拟大量节点/集群，适合调度和控制面扩展测试。 |
| `node-feature-discovery` | P1 | 发现节点硬件能力，连接 GPU、NUMA、加速器调度。 |
| `kube-scheduler-simulator` | P1 | 可视化/模拟 scheduler 行为。 |
...
| 项目 | 优先级 | 为什么值得加入 |
|---|---|---|
| `metrics-server` | P0 | HPA/VPA 的基础资源指标源，K8s autoscaling 入口。 |
| `prometheus-adapter` | P0 | 把 Prometheus 指标暴露为 custom/external metrics API，连接高级 autoscaling。 |
| `inference-perf` | P1 | GenAI inference performance benchmarking tool，可与 `llm-d-benchmark` 对比。 |
| `headlamp` | P1 | K8s UI / debugging / monitoring，可和 [[kubewall]]、`k8m` 对比。 |
...
| 项目 | 优先级 | 为什么值得加入 |
|---|---|---|
| `kind` | P0 | 本地 Kubernetes in Docker，是 controller/operator 开发和 CI 的事实基线。 |
| `kubespray` | P0 | 生产集群部署自动化，适合理解集群生命周期和裸金属/on-prem。 |
| `cri-tools` | P0 | CRI CLI + validation，理解 kubelet/runtime 边界。 |
| `security-profiles-operator` | P1 | seccomp/AppArmor/SELinux profile operator，连接 runtime security。 |
| [[agent-sandbox]] | 已有 | 已收录，AI Agent runtime 的 sandbox CRD。 |
| `lws` | P1 | LeaderWorkerSet，用一组 Pod 表达 leader/worker 分布式 workload。 |
| `jobset` | P1 | 分布式 ML/HPC workload API，和 `Kueue`、batch serving 强相关。 |

### API / Operator / 控制器开发

| 项目 | 优先级 | 为什么值得加入 |
|---|---|---|
| `controller-runtime` | P0 | Manager、cache、client、reconcile、webhook、envtest 的现代 controller 抽象层。 |
| `kubebuilder` | P0 | CRD/controller 项目脚手架和代码生成路径。 |
| `controller-tools` | P0 | CRD、RBAC、webhook、object deepcopy 等生成工具链。 |
| `cluster-api` | P0 | 声明式集群生命周期管理，适合接多集群和平台工程。 |
| `kustomize` | P1 | Kubernetes YAML 定制工具链，GitOps/配置管理基础。 |
| `kro` | P1 | Kube Resource Orchestrator，适合与 Crossplane Composition / higher-level API 对比。 |
...
| 项目 | 优先级 | 为什么值得加入 |
|---|---|---|
| `mcp-lifecycle-operator` | P1 | 声明式管理 MCP Servers，和 [[mcp]]、[[agentgateway]]、[[declarative-agent-management]] 直接相关。 |
| `kube-agentic-networking` | P1 | Agent/tool 网络策略治理，适合补 Agent runtime 安全边界。 |
| `ai-conformance` | P2 | AI conformance definition/proposals/tests，待生态更稳定后再摄入。 |
| `inference-perf` | P1 | GenAI inference benchmark，与 [[llm-d]]、[[aibrix]]、[[kserve]] 有直接交集。 |
...
- 网络专项：`aws-load-balancer-controller`、`ingress2gateway`、`apiserver-network-proxy`、`dranet`。
- 存储专项：`aws-ebs-csi-driver`、`aws-efs-csi-driver`、`nfs-subdir-external-provisioner`、`sig-storage-local-static-provisioner`。
- 安全专项：`security-profiles-operator`、`secrets-store-csi-driver`、`kube-agentic-networking`。
- 工具/测试专项：`krew`、`kustomize`、`e2e-framework`、`kubetest2`。
