...
Agent Resume 是一个本地优先的多 Agent 上下文接续工具。它帮助用户在 Claude Code、Codex、OpenCode 等编码 Agent 之间迁移工作上下文，把历史对话、项目状态、用户诉求、关键决策、约束条件和待办事项提炼成结构化的上下文简报。

第一版的主要体验不是新的终端 TUI，而是在当前 Agent 对话中显式调用 `/agent-resume`。CLI 和 MCP server 是底层能力，供 skills、slash command、command wrapper 或 Agent 指令模板调用。

## 产品目标

用户经常在多个编码 Agent 之间切换。Claude Code 的某个对话里可能已经包含了用户诉求、设计决策和未完成问题，但新开的 Codex 对话看不到这些上下文。用户不希望复制完整聊天记录，也不希望反复解释“上次做到哪里了”。

Agent Resume 要解决的问题是：
...
Agent Resume 应作为独立项目开发，不直接放在 `llm-wiki` 内实现。`llm-wiki` 只保存需求文档、架构分析和后续知识库条目。

默认实现目录：

```text
/Users/zhenyu.jiang/agent-resume
```
...
```text
/agent-resume
/agent-resume current
/agent-resume brief
/agent-resume full
/agent-resume search "claude codex memory"
/agent-resume capture
/agent-resume capture "我们决定 V1 MCP 只读，写入走 CLI review。"
```
...
3. Agent Resume adapter 维护的 Agent history index。
...
```bash
agent-resume capture
agent-resume capture --notes "..."
agent-resume capture --file transcript.md
agent-resume capture --source claude-code
agent-resume review
agent-resume brief
agent-resume brief --target codex --level standard
agent-resume search "query"
agent-resume list
```
...
- 提供 `/agent-resume` 的 skill、command 或指令模板。
- 调用 CLI 或 MCP server。
...
- `/agent-resume` 默认搜索当前项目 + 用户记忆。
- `/agent-resume search` 优先搜索当前项目，需要时可扩展到 all projects。
- `/agent-resume current` 只加载当前项目和用户记忆。

## 端到端示例

1. 用户在 Claude Code 中讨论并决定：Agent Resume V1 使用 CLI core、Agent 内命令体验、MCP 只读。
2. 用户输入 `/agent-resume capture "我们决定命令叫 /agent-resume，主体验不是独立 TUI。"`
3. Agent 展示候选记忆 diff。
4. 用户接受候选条目。
5. 用户稍后在同一项目中打开 Codex，输入 `/agent-resume`。
6. Codex 列出相关上下文简报。
7. 用户选择 Agent Resume 设计简报。
...
- 用户能在支持的 Agent 中输入 `/agent-resume` 并看到相关上下文选择。
...
- 项目目录 `.agent-resume/` 默认是否写入 `.gitignore`，还是拆成可共享和私有两个子目录？
- 各 Agent adapter 如何发现历史记录？无法读取原生 transcript 时，history index 的最小格式是什么？
- `/agent-resume capture` 默认先展示来源选择，还是默认 capture 当前对话 notes？
